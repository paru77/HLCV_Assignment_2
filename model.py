...
class TwoLayerNetv2(TwoLayerNetv1):
    ...
    def compute_loss(self, X, y=None, reg=0.0):
        ...
        correct_logprob=-np.log(softmax_scores[np.arange(N),y]) 
        # print(correct_logprob)
        data_loss=np.sum(correct_logprob)/N
        # print(data_loss)
        reg_loss=reg*(np.sum(self.params['W1']*self.params['W1'])+np.sum(self.params['W2']*self.params['W2']))
        # print(reg_loss)
